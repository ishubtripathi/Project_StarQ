...
export default function Chat() {
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: "welcome",
      role: "assistant",
      content: "Hello! I'm StarQ. Ask me anything about your documents.",
    },
  ]);
  ...
  const handleSend = async (content: string) => {
    ...
    if (!activeDocument || activeDocument.documents.length === 0) {
      const errorMessage: ChatMessage = {
        id: `${Date.now()}-error`,
        role: "assistant",
        content: "Please upload or select a document before asking a question.",
      };
    ...
    try {
      ...
      const response = await sendQuery(
        trimmedContent,
        activeDocument.documents.map((document) => document.document_id),
      );
